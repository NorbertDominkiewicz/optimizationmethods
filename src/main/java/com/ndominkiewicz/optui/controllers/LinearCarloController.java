...
public class LinearCarloController implements ViewController, Initializable {
    private GraphicsContext graphicsContext;
    private final int canvasWidth = 400;
    private final int canvasHeight = 400;
    private MonteCarlo algorithm;
    /* FXML */
    @FXML
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        algorithm = new MonteCarlo(canvasWidth, canvasHeight, result, time);
        loadUpActions();
    ...
    public void runAlgorithm() {
        ...
        timeline.setOnFinished(event -> {
            ...
            graphicsContext.setLineWidth(1);
            graphicsContext.strokeRect(0, 0, canvasWidth, canvasHeight);
            algorithm.runLinearly(Integer.parseInt(input.getText()), graphicsContext);
        });
        timeline.play();
    }
}
